    ...
    <PageShell title={mode === 'edit' ? t('announcements.edit') : t('announcements.add')} subtitle={t('announcements.formSubtitle')} backLabel={t('announcements.backToList')} onBack={onBack}>
      <SectionCard title={t('announcements.details')} action={<StatusBadge status={data.status} />}>
        ...
        <div className="mt-6 flex flex-wrap justify-end gap-2">
          ...
          <button onClick={onSave} className="inline-flex h-10 items-center gap-2 rounded-lg bg-indigo-600 px-4 text-sm font-medium text-white shadow-sm hover:bg-indigo-700"><Save className="h-4 w-4" />{t('announcements.save')}</button>
          <button onClick={onSaveAndClose ?? onSave} className="inline-flex h-10 items-center gap-2 rounded-lg bg-slate-900 px-4 text-sm font-medium text-white shadow-sm hover:bg-slate-800"><Save className="h-4 w-4" />{t('common.saveAndClose')}</button>
        </div>
      </SectionCard>
    </PageShell>
  );
