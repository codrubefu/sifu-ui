...
function Kpi({ label, value, helper }: { label: string; value: string; helper?: string }) {
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-4">
      <p className="text-xs font-medium normal-case text-slate-500">{label}</p>
                ...
                {chartData.length ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={chartData}>
                      ...
                      <Bar dataKey="total" fill="#237c68" radius={[4, 4, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
        ...
        <SectionCard title={t('reports.segments')} action={<Button onClick={() => void loadSegments()} disabled={segmentsLoading}><RefreshCw className="h-4 w-4" />{t('common.refresh')}</Button>}>
                ...
                <tbody>
                  {segments.length ? segments.map((segment) => (
                    <tr key={segment.id} className="border-b border-slate-100 align-top transition-colors hover:bg-indigo-50/30">
                      <td className="px-5 py-3 font-medium text-slate-900">{segment.name}</td>
          ...
          {segmentMembersLabel ? (
            <div className="mt-5 rounded-lg border border-slate-200 bg-slate-50 p-4">
              <h3 className="text-sm font-medium text-slate-900">{t('reports.previewFor', { name: segmentMembersLabel })}</h3>
