          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div className="md:col-span-2 rounded-lg border border-slate-200 bg-slate-50 p-4">
                ...
                {needsPayment ? <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2"><TextField label={t('events.paymentAmount')} type="number" min={0} step="0.01" value={form.payment_amount ?? ''} onChange={(e) => updateField('payment_amount', e.target.value ? Number(e.target.value) : null)} error={fieldError(errors, 'payment_amount')} /><TextField label={t('services.currency')} maxLength={3} value={form.payment_type ?? 'RON'} onChange={(e) => updateField('payment_type', e.target.value.toUpperCase())} error={fieldError(errors, 'payment_type')} placeholder="RON" /></div> : <p className="mt-2 text-sm text-slate-500">{t('events.paymentFieldsClearedHint')}</p>}
              </div>
            </div>
          </div>
        </div>
